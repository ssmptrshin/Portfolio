...
class Game {
    ...
    void process() {
        Player white, black; white = black = null; doClear(null);
        _whiteIsManual = true; _blackIsManual = false;
        while (true) {
            _state = SETUP;
            while (_state == SETUP) {
                try {
            ...
            while (_state != SETUP && !_board.gameOver()) {
                ...
                move = createMove(move, white, black);
                if (_state == PLAYING) {
                    if (move == null) {
                        break;
                    }
                    if (_board.legalMove(move)) {
                        ...
                        if (_board.whoseMove() == WHITE && !_whiteIsManual) {
                            continue;
                        } else if (_board.whoseMove()
     ...
     * @return the move
     */
    Move createMove(Move move, Player white, Player black) {
    ...
    /** Does the printing of the moves.
     *
     * @param move the move that is being made
     */
    void doAIAction(Move move) {
        if (_board.getBoard()
                [Move.index(move.col0(), move.row0())] == WHITE
                && !_whiteIsManual) {
            System.out.println("White moves "
                    + move.toString() + ".");
        } else if (_board.getBoard()
                [Move.index(move.col0(), move.row0())] == BLACK
                && !_blackIsManual) {
    ...
    void doSet(String[] operands) {
        ...
        if (operands[0].equalsIgnoreCase("Black")) {
            color = BLACK;
        }
        _board.clear();
    ...
    void doDump(String[] unused) {
        String printedBoard = _board.toString();
        System.out.println("===");
