...
class Move {
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.jumpTail() == null) {
            return move(move0.col0(), move0.row0(),
                    move1.col1(), move1.row1(), move1);
        } else {

            if (move1.jumpTail() != null) {
                move(move1, move1.jumpTail());
            }

            Move newJumpTail = move(move0.jumpTail().col0(),
                    move0.jumpTail().row0(),
                    move0.jumpTail().col1(),
                    move0.jumpTail().row1(), move1);

            return move(move0.col0(), move0.row0(),
                    move0.col1(), move0.row1(), newJumpTail);
        }
    }
    ...
    private void toString(Formatter out) {
        out.format("%c%c-%c%c",
                col0(), row0(), col1(), row1());
        Move copy = this._nextJump;
        while (copy != null) {
            out.format("-%c%c", copy.col1(), copy.row1());
            copy = copy._nextJump;
        }
    }

    /** Set me to COL0 ROW0 - COL1 ROW1 - NEXTJUMP. */
    public void set(char col0, char row0, char col1, char row1,
                     Move nextJump) {
